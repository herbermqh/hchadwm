...
batt() {
  ...
  printf "^c#81A1C1^ $(acpi | sed "s/%//g" | sed "s/,//g" |xargs | awk '{if ($3 == "Discharging"){print "Discharging"} else {if ($3 == "Full"){print "Charged"}else{print "Charging"}}}')"
...
wlan() {
  ...
  up) printf "^c#3b414d^ ^b#7681c5^ 􏇫 ^d^%s" " ^c#828dd1^Connected" ;;
  down) printf "^c#3b414d^ ^b#7681c5^ 睊 ^d^%s" " ^c#828dd1^Disconnected" ;;
  esac
}
...
while true; do
  ...
  sleep 1 && xsetroot -name "$(temperature_cpu) $(batt) $(brightness) $(cpu) $(mem) $(wlan) $(clock)"
